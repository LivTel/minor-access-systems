 ...
 */
public class GCN_Client2 extends Thread {
	...
	GCN_Client2 client = new GCN_Client2(host, port, delay);
	...
	System.err.println("USAGE: java GCN_Client2 [options]"+
    ...
    public GCN_Client2(String host, int port, long delay) {
	...
	public void writeAlert() throws IOException {
		...
		writeSod(now);     // 3
		byteDataOut.writeInt(1);   // 4 - trig_seq_num
		byteDataOut.writeInt(11910+date.getDate()); // 5 - burst_tjd
		byteDataOut.writeInt((int)(now - 150L)); // 6 - burst_sod
		writeStuff(7, 8); // 7, 8 - spare 
		//int trig_flags = GAMMA_TRIG | WXM_TRIG | PROB_GRB;
		int trig_flags = 11;
		byteDataOut.writeInt(type); // 9 - trig_flags
		byteDataOut.writeInt(350);   // 10 - gamma_cnts
		byteDataOut.writeInt(22786); // 11 - wxm_cnts
		byteDataOut.writeInt(5467);  // 12 - sxc_cnts
		byteDataOut.writeInt((int)(now - 2450L)); // 13 - gamma_time
		byteDataOut.writeInt((int)(now - 2435L)); // 14 - wxm_time
		byteDataOut.writeInt((240 << 16) | 75); // 15 - sc_point
	...
	public void write() throws IOException {
	   ...
	   // writeInt(burstTJD); // 5
	    ...
	    byteDataOut.writeInt(trigFlags); // 9
	    int gammaCts = 1000;
	    byteDataOut.writeInt(gammaCts); // 10
	    int wxmCts = 500;
	    byteDataOut.writeInt(wxmCts); // 11
	    int sxcCts = 400;
	    byteDataOut.writeInt(sxcCts); // 12
	    int gammaTime = 2000;
	    byteDataOut.writeInt(gammaTime); // 13
	    int wxmTime = 1900;
	    byteDataOut.writeInt(wxmTime); // 14
	    int scRA = 250000; // 25degs
	    int scDec = 150000; // +15degs
	    byteDataOut.writeInt((scRA << 16) & scDec); // 15
	    int wxRA1  = burstRA;
	    int wxRA2  = burstRA;
	    int wxRA3  = burstRA;
	    int wxRA4  = burstRA;
	    ...
	    int wxDec3 = burstDec;
	    int wxDec4 = burstDec;
	    byteDataOut.writeInt(wxRA1); // 16
	    byteDataOut.writeInt(wxDec1); // 17
	    byteDataOut.writeInt(wxRA2); // 18
	    byteDataOut.writeInt(wxDec2); // 19
	    byteDataOut.writeInt(wxRA3); // 20
	    byteDataOut.writeInt(wxDec3); // 21
	    byteDataOut.writeInt(wxRA4); // 22
	    byteDataOut.writeInt(wxDec4); // 23
	    int wxErrors = (30 << 16) & 5;
	    byteDataOut.writeInt(wxErrors); // 24
	    int wxDmSig = 50;
	    byteDataOut.writeInt(wxDmSig); // 25
	    int sxRA1 = burstRA;
	    int sxRA2 = burstRA;
	    int sxRA3 = burstRA;
	    int sxRA4 = burstRA;
	    ...
	    int sxDec3 = burstDec;
	    int sxDec4 = burstDec;
	    byteDataOut.writeInt(sxRA1); // 26
	    byteDataOut.writeInt(sxDec1); // 27
	    byteDataOut.writeInt(sxRA2); // 28
	    byteDataOut.writeInt(sxDec2); // 29
	    byteDataOut.writeInt(sxRA3); // 30
	    byteDataOut.writeInt(sxDec3); // 31
	    byteDataOut.writeInt(sxRA4); // 32
	    byteDataOut.writeInt(sxDec4); // 33
	    int sxErrors = (3 << 16) & 1;
	    byteDataOut.writeInt(sxErrors); // 34
	    int sxDmSig = 30;
	    byteDataOut.writeInt(sxDmSig); // 35
	    int posFlags = 1;
	    byteDataOut.writeInt(posFlags); // 36
	    int valid = 1; // BURST_VALID.
	    byteDataOut.writeInt(valid); // 37
	    writeStuff(38,38); // 38
	    writeTerm(); // 39
	    // write whole packet out in one go.
	    dataOut.write(byteOut.toByteArray());
	...
	public void writeSax() throws IOException {
	    ...
	    byteDataOut.writeInt(11910+date.getDate()); // 5 - burst_tjd
	    byteDataOut.writeInt((int)(date.getTime() - 150L)); // 6 - burst_sod
	    byteDataOut.writeInt((int)(ra*10000.0)); // 7 - burst RA [ x10000].
	    byteDataOut.writeInt((int)(dec*10000.0)); // 8 - burst Dec [x10000].
	    byteDataOut.writeInt((int)(Math.random()*10000.0)); // 9 - burst intens mCrab.
	    writeStuff(10, 10);   // 10 - spare
	    byteDataOut.writeInt((int)(error*10000.0)); // 11 - burst error
	    byteDataOut.writeInt((int)(Math.random()*10000.0)); // 12 - burst conf [% x 100].
	    writeStuff(13, 17); // 13,, 17 - spare.
	    //int trig_id = SUSP_GRB;
	    byteDataOut.writeInt(type); // 18 - trigger flags.
	    byteDataOut.writeInt(5); // 19 - stuff.
